...
//        localVideoView.addEventListener("loadedmetadata", scaleToFill);
		...
		$("#localContainer").append(localVideoView);
...
//  videoView.style.position = 'absolute';
  ...
  $("#videoContainer").html("");
  $("#videoContainer").append(videoView);
...
function refreshVideoContainer() {
  ...
  var videoHeight = videoConfig.containerParams.size[1];
  ...
  for (var row = 0; row < rows && videoViewIndex < n; row++) {
    ...
    for (var video = 0; video < videosInRow && videoViewIndex < n; video++) {
      ...
      videoView.style.height = videoHeight + 'px';
...
function refreshLocalVideoView() {
  ...
  //localVideoView.style.left = (videoConfig.containerParams.position[0] + videoConfig.local.position[0]) + 'px';

 //localVideoView.style.top = (videoConfig.containerParams.position[1] + videoConfig.local.position[1]) + 'px';       
}
